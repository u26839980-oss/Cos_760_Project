...
QUICKSTART - run these from the repository root:

  # Test everything works (no audio needed):
  python src/pipeline.py --mode demo

  # Setswana audio, skip slow engines:
  python src/pipeline.py --mode real --language tsn --skip-mms

  # Setswana audio, all engines:
  python src/pipeline.py --mode real --language tsn

  # isiZulu audio, no Lelapa key yet:
  python src/pipeline.py --mode real --language zul --skip-lelapa

  # isiZulu audio, all engines (need LELAPA_API_KEY set):
  python src/pipeline.py --mode real --language zul
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(PROJECT_ROOT, "pipeline.log"), encoding="utf-8"),
...
DIRS = {
    "audio":       os.path.join(SRC_DIR, "data", "audio"),
    "transcripts": os.path.join(SRC_DIR, "data", "transcripts"),
    "figures":     os.path.join(SRC_DIR, "outputs", "figures"),
    "results":     os.path.join(SRC_DIR, "outputs", "results"),
    "coherence":   os.path.join(SRC_DIR, "outputs", "coherence"),
}
...
def stage_asr(args) -> dict:
    ...
    if args.mode == "demo":
        ...
        return generate_demo_transcripts(
    ...
    audio_files = sorted(
        glob.glob(os.path.join(DIRS["audio"], "*.mp3")) +
        glob.glob(os.path.join(DIRS["audio"], "*.wav")) +
        glob.glob(os.path.join(DIRS["audio"], "*.ogg")) +
        glob.glob(os.path.join(DIRS["audio"], "*.m4a"))
    )
    if not audio_files:
        logger.error(f"No files in {DIRS['audio']}. Add audio or use --mode demo")
        sys.exit(1)
    ...
    for eng, texts in by_engine.items():
        ...
        if w < 20:
            logger.warning(
                f"  [{eng}] Very few words - check {DIRS['transcripts']} for errors.\n"
...
def main():
    ...
    if not all_res:
        logger.error(
            ...
            f"  1. ASR transcripts are empty - check {DIRS['transcripts']}\n"
    ...
    logger.info("\n" + "=" * 60)
    logger.info(f"  DONE. Check {os.path.join(SRC_DIR, 'outputs')} for all results.")
    logger.info("=" * 60)
